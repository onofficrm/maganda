...
MG_BOOT = (
    ...
    'function mgTiktokUser(u){if(!u)return"";u=String(u).trim();const m=u.match(/tiktok\\.com\\/@([^/?\\s&]+)/i);return m?m[1]:""}'
    'function mgInstagramUser(u){if(!u)return"";u=String(u).trim();const m=u.match(/instagram\\.com\\/([^/?\\s#]+)/i);if(!m)return"";const n=m[1].toLowerCase();return["p","reel","reels","stories","tv","live","explore"].includes(n)?"":m[1]}'
    'function mgStreamPlatform(item){if(!item)return"";if(item.platform)return item.platform;const u=item.stream_url||item.youtube_id||"";if(mgYoutubeId(u||item.embed_url))return"youtube";if(mgTiktokUser(item.tiktok_url||u))return"tiktok";if(mgInstagramUser(item.instagram_url||u))return"instagram";return""}'
    'function mgStreamWatchUrl(item){if(!item)return"#";const p=mgStreamPlatform(item);if(p==="youtube")return item.stream_url||("https://www.youtube.com/watch?v="+mgYoutubeId(item.youtube_id||item.stream_url||item.embed_url));if(p==="tiktok")return item.tiktok_url||item.stream_url||"#";if(p==="instagram")return item.instagram_url||item.stream_url||"#";return item.stream_url||"#"}'
    'function mgStreamEmbedSrc(item){if(!item)return"";if(item.embed_url&&item.platform&&item.platform!=="youtube")return item.embed_url;const p=mgStreamPlatform(item);if(p==="youtube")return mgYoutubeEmbedUrl(item.youtube_id||item.stream_url||item.embed_url);if(p==="tiktok"){const user=mgTiktokUser(item.tiktok_url||item.stream_url);return user?"https://www.tiktok.com/embed/@/"+encodeURIComponent(user):""}if(p==="instagram"){const user=mgInstagramUser(item.instagram_url||item.stream_url);return user?"https://www.instagram.com/"+encodeURIComponent(user)+"/embed":""}return item.embed_url||""}'
    'function mgStreamPlatformLabel(p){return ({youtube:"YouTube",tiktok:"TikTok",instagram:"Instagram"})[p]||"Live"}'
    'function mgCreatorPayload(item){if(!item)return null;const p=mgStreamPlatform(item)||item.platform||"";return{id:item.id,slug:item.slug,name:item.name||item.streamer,title:item.title,subtitle:item.intro||item.subtitle,avatar:item.avatar||item.profile||item.image,cover:item.cover||item.thumb,platform:p,stream_url:item.stream_url,youtube_id:item.youtube_id||mgYoutubeId(item.stream_url),tiktok_url:item.tiktok_url||"",instagram_url:item.instagram_url||"",embed_url:item.embed_url||mgStreamEmbedSrc(item),stream_label:item.stream_label||mgStreamPlatformLabel(p),is_live:!!(item.is_live||item.isLive)}}'
...
JP_EMBED_NEW = (
    '(()=>{const p=mc&&mgStreamPlatform(mc);const src=mc&&mgStreamEmbedSrc(mc);const href=mc&&mgStreamWatchUrl(mc);'
    'if(!src)return c.jsxs("div",{className:"absolute inset-0 w-full h-full flex items-center justify-center bg-black/80 text-white p-6 text-center",children:[c.jsx("img",{src:(mc&&mc.cover)||"https://images.unsplash.com/photo-1540202404-b71114227fe1?w=1600&q=80",className:"absolute inset-0 w-full h-full object-cover opacity-40",alt:"offline"}),c.jsxs("div",{className:"relative z-10",children:[c.jsx("p",{className:"text-lg font-bold mb-2",children:mgT("방송 준비중",F)}),c.jsx("p",{className:"text-sm text-white/80",children:mgT("관리자에서 YouTube / TikTok / Instagram URL을 등록해 주세요.",F)})]})]});'
    'return c.jsxs("div",{className:"absolute inset-0 w-full h-full bg-black",children:['
    'c.jsx("iframe",{src:src,className:"absolute inset-0 w-full h-full border-0",allow:"autoplay; encrypted-media; picture-in-picture; fullscreen",allowFullScreen:!0,title:(mc&&mc.name)||mgStreamPlatformLabel(p)}),'
    ...
    'c.jsx("a",{href:href,target:"_blank",rel:"noopener noreferrer",className:"absolute top-3 right-3 z-20 px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 text-white text-xs font-medium backdrop-blur",children:mgStreamPlatformLabel(p)})'
    ']})})()'
)
