...
if (function_exists('g5site_cfg')) {
    $maganda_home_builder_id = g5site_cfg('home_builder_bridge_id', '');
    if ($maganda_home_builder_id !== '') {
        $maganda_home_builder_id = preg_replace('/[^a-z0-9_-]/i', '', $maganda_home_builder_id);
        if ($maganda_home_builder_id !== '') {
            if (!defined('ONOFF_BUILDER_LOADED') && defined('G5_PLUGIN_PATH')) {
                $maganda_builder_bootstrap = G5_PLUGIN_PATH . '/onoff-builder-bridge/bootstrap.php';
                if (is_file($maganda_builder_bootstrap)) {
                    include_once $maganda_builder_bootstrap;
                }
            }
            if (function_exists('onoff_builder_render_import_page')) {
                onoff_builder_render_import_page($maganda_home_builder_id);
                exit;
            }
        }
    }
